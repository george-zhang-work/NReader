...
import com.nreader.BuildConfig;
...
/**
 * This class is used to holds bitmap caches in memory and disk.
 */
public class ImageCache {
	private static final String TAG = "ImageCache";

	/**
	 * Default memory cache size for images. (5Mib)
	 */
	private static final int DEFAULT_MEM_CACHE_SIZE = 1024 * 1024 * 5;

	/**
	 * Default disk cache size for images. (10Mib)
	 */
	...
	/**
	 * Hint to the compressor, 0-100. 0 meaning compress for small size, 100 meaning
	 * compress for max quality. Some formats, like PNG which is lossless, will ignore the
	 * quality setting
	 */
	...
	/**
	 * Handle Configuration Changes. Find and return an existing ImageCache stored in a
	 * {@link RetainFragment}, if not found a new one is created using the supplied params
	 * and saved to a {@link RetainFragment}.
	 * 
	 ...
	 * disk cache when it is created, instead you should call initDiskCache() to
	 * initialize it on a background thread.
	 */
	...
	/**
	 * To check whether the data is in memory cache.
	 * 
	 * @param data Unique identifier for which item to get
	 * @return true if the data is in the memory cache, otherwise false.
	 */
	public boolean checkBitmapExistInMemCache(String data) {
		boolean res = false;
		if (mMemoryCache != null) {
			res = mMemoryCache.get(data) != null;
		}
		return res;
	}
	...
	/**
	 * Get bitmap from disk cache. It should be called in a worker thread.
	 * 
	 * @param data Unique identifier for which item to get
	 * @return The bitmap if found in cache, null otherwise
	 */
	public Bitmap getBitmapFromDiskCache(String data, boolean isForPreload) {
		final String key = hashKeyForDisk(data);
		synchronized (mDiskCacheLock) {
	...
	/**
	 * To check whether the data is in disk cache.
	 * 
	 * @param data Unique identifier for which item to get
	 * @return true if the data is in the disk cache, otherwise false.
	 */
	public boolean checkBitmapExistInDiskCache(String data) {
		final String key = hashKeyForDisk(data);
		synchronized (mDiskCacheLock) {
			while (mDiskCacheStarting) {
				try {
	...
	/**
	 * Clears both the memory and disk cache associated with this ImageCache object. Note
	 * that this includes disk access so this should not be executed on the main/UI
	 * thread.
	 */
	...
	/**
	 * Flushes the disk cache associated with this ImageCache object. Note that this
	...
	/**
	 * Closes the disk cache associated with this ImageCache object. Note that this
	 * includes disk access so this should not be executed on the main/UI thread.
	 */
	public void close() {
		synchronized (mDiskCacheLock) {
	...
	public static class ImageCacheParams {
		 ...
		 * Eg. setting percent to 0.2 would set the memory cache to one fifth of the
		 * device memory class. Throws {@link IllegalArgumentException} if percent is <
		 * 0.05 or > .8. This value should be chosen carefully based on a number of
	...
	/**
	 * A hashing method that changes a string (like a URL) into a hash suitable for using
	 * as a disk filename.
	 */
	...
	/**
	 * Locate an existing instance of this Fragment or if not found, create and add it
